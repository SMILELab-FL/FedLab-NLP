...
class SerialTrainer(ClientTrainer):
    ...
    def local_process(self, id_list, payload):
        ...
        Args:
            id_list (list[int]): BaseClient id in this trainers serial.
            payload (list[torch.Tensor]): communication payload from server.
        """
        self.param_list = []
        model_parameters = payload[0]
        self._LOGGER.info(
            "Local trainers with client id list: {}".format(id_list))
        for idx in id_list:
            self._LOGGER.info(
                "Starting trainers procedure of client [{}]".format(idx))
...
class SubsetSerialTrainer(SerialTrainer):
    ...
    def _get_dataloader(self, client_id):
        """Return a trainers dataloader used in :meth:`train` for client with :attr:`id`
    ...
    def _train_alone(self, model_parameters, train_loader):
        """Single round of local trainers for one client.

        Note:
            Overwrite this method to customize the PyTorch trainers pipeline.
