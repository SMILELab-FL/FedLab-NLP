...
class GlueDataLoader(BaseDataLoader):
    ...
    def _load_federated_data_on_server(self):
        if os.path.isfile(self.cached_data_file):
            ...
            train_features_dict, valid_features_dict, valid_fedtures_all, \
            train_examples_num_dict, valid_examples_num_dict, train_num, valid_num = pickle_read(self.cached_data_file)
            # server doesn't use each client's train & test dataset
            del train_features_dict, valid_features_dict

        else:
            self.logger.info(f"create clients data ...")
            ...
            if n_clients != self.federated_config.clients_num:
                raise ValueError(f"partition data have {n_clients} clients "
                                 f"that mismatch you input {self.federated_config.clients_num} clients")
            ...
            train_num, valid_num = len(train_features_all), len(valid_fedtures_all)

            federated_data = (
                train_features_dict, valid_features_dict, valid_fedtures_all,
                train_examples_num_dict, valid_examples_num_dict, train_num, valid_num
            )
        ...
        self.test_dataloader = self.build_dataload(valid_fedtures_all, "valid")
        self.train_examples_num_dict = train_examples_num_dict
        self.valid_examples_num_dict = valid_examples_num_dict
        self.train_num, self.valid_num = train_num, valid_num
    ...
    def _load_federated_data_on_client(self):
        ...
        if self.federated_config.do_mimic:
            ...
            train_features_dict, valid_features_dict, valid_fedtures_all, \
            train_examples_num_dict, valid_examples_num_dict, train_num, valid_num = pickle_read(self.cached_data_file)
            del valid_fedtures_all

            for idx in self.clients_list:
                train_dataloader_dict[idx] = self.build_dataload(train_features_dict[idx], "train")
                valid_dataloader_dict[idx] = self.build_dataload(valid_features_dict[idx], "valid")
    ...
    def build_dataload(self, features, mode="train"):
